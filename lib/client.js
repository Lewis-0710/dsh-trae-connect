		...
		*/
		function apply(ctx) {
			try {
				...
				let quotaScope;
				try {
					const configForms = resolveService(ctx, "configForms");
					const settingsScope = resolveService(ctx, "settingsScope");
					const scope = configForms && typeof configForms.get === "function" ? configForms.get("trae-quota") : settingsScope && typeof settingsScope.bind === "function" ? settingsScope.bind({ namespace: "trae-quota" }) : void 0;
					quotaScope = scope;
					if (scope) {
						const applySnapshot = () => {
